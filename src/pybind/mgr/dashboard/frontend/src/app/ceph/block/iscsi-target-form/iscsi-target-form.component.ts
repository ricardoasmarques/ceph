...
export class IscsiTargetFormComponent implements OnInit {
  ...
  submit() {
    ...
    if (this.cephIscsiConfigVersion > 10) {
      ...
      request['auth'] = {
        user: targetAuth.getValue('user'),
        password: targetAuth.getValue('password'),
        mutual_user: targetAuth.getValue('mutual_user'),
        mutual_password: targetAuth.getValue('mutual_password')
      };
    }
