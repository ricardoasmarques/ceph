...
export class RbdFormComponent implements OnInit {
    ...
    if (this.editing) {
      ...
      this.routeParamsSubscribe = this.route.params.subscribe(
        (params: { pool: string, name: string }) => {
          ...
          this.rbdService.get(poolName, rbdName).then((resp: RbdFormResponseModel) => {
  ...
  editAction() {
    ...
    this.rbdService.update(this.response.pool_name, this.response.name, request).then((resp) => {
      if (resp.status === 202) {
        this.notificationService.show(NotificationType.info,
